...
MAX_LEN = 10
arr = deque(maxlen=MAX_LEN)
pts = deque(maxlen=MAX_LEN)
...
while True:
    ...
    if not getROI:
        ...
        print lower
        # rgb_min = cv2.cvtColor(lower,cv2.COLOR_HSV2RGB)
        # rgb_max = cv2.cvtColor(upper,cv2.COLOR_HSV2RGB)
        # print rgb_min
        # print rgb_max
    ...
    if (len(arr) == 10):
        ...
        if checkPosition(arr[0]) == 1 and old_position != 1:
            count += 1
            print "left  ", count
            wsh.SendKeys("{down}")  # seaand the keys you want

        if checkPosition(arr[0]) == 2 and old_position != 2:
            count += 1
            print "right ", count
            wsh.SendKeys("{up}")  # seaand the keys you wantaa
        old_position = checkPosition(arr[0])
    ...
    for i in xrange(1, len(pts)):
        ...
        thickness = int(np.sqrt(MAX_LEN / float(i + 1)) * 2.5)
        cv2.line(frame, pts[i - 1], pts[i], (0, 0, 255), thickness)
        cv2.line(frame, (100, 0), (100, 600), (0, 0, 255))
        cv2.line(frame, (500, 0), (500, 600), (0, 0, 255))
